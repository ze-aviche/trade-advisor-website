...
@app.route('/api/chart/bars', methods=['GET'])
@require_auth
def get_chart_bars():
    """Return OHLCV bars for a symbol from Alpaca for Lightweight Charts rendering.

    Query params:
        symbol  — ticker (required)
        period  — 1D (1-min, today) | 5D (5-min, last 5 days) |
                  1M (daily, 1 month) | 3M (daily, 3 months) | 1Y (weekly, 1 year)
    """
    ...
    if period not in ('1D', '5D', '1M', '3M', '1Y'):
        return jsonify({'error': 'Invalid period'}), 400

    try:
        user_id = getattr(request.user, 'id', 1)
        broker = _brown_broker or _get_broker(user_id)
        if not broker or not hasattr(broker, '_data_client') or not broker._data_client:
            return jsonify({'error': 'No data client — broker not connected'}), 503
        ...
        if period == '1D':
            ...
            start = now_utc - timedelta(days=31)
        elif period == '3M':
            tf    = TimeFrame(1, TimeFrameUnit.Day)
            start = now_utc - timedelta(days=92)
        else:  # 1Y
            tf    = TimeFrame(1, TimeFrameUnit.Week)
            start = now_utc - timedelta(days=366)

        req  = StockBarsRequest(symbol_or_symbols=symbol, timeframe=tf,
                                start=start, end=now_utc, adjustment='all')
        data = broker._data_client.get_stock_bars(req)
        raw  = data.get(symbol, [])
        ...
        for b in raw:
            ts = b.timestamp
            t  = int(ts.timestamp()) if hasattr(ts, 'timestamp') else int(datetime.fromisoformat(str(ts)).timestamp())
            bars.append({'time': t, 'open': float(b.open), 'high': float(b.high),
                         'low': float(b.low), 'close': float(b.close), 'volume': float(b.volume)})

        return jsonify({'bars': bars, 'symbol': symbol, 'period': period})

    except Exception as e:
        app_logger.warning(f'[chart/bars] {symbol} {period}: {e}')
