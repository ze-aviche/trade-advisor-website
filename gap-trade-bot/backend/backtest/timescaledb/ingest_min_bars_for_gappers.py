...
MAX_WORKERS = 4                 # reduced for better rate limit compliance
...
def fetch_existing_days(ticker_dates: List[Tuple[str, str]]):
    """Return a set of (ticker, day) already present (fast approach: query DB once for the date range)"""
    # Build unique set of days to query
    days = sorted({d for _, d in ticker_dates})
    if not days:
        return set()
    ...
    with get_db_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT DISTINCT ticker, day FROM ohlcv_1m
                WHERE day >= %s AND day <= %s
            """, (min_day, max_day))
            rows = cur.fetchall()
    return set((r[0], r[1].isoformat() if isinstance(r[1], datetime) else str(r[1])) for r in rows)
...
def run_orchestrator(ticker_dates: List[Tuple[str, str]]):
    ...
    # remove ones already in DB
    existing = fetch_existing_days(uniq)
    tasks = [td for td in uniq if td not in existing]
    print(f"To fetch (not already in DB): {len(tasks)}")

    results = []
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        futures = {ex.submit(process_one, t, d): (t,d) for t,d in tasks}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Ingest minute bars for gap tickers from SQLite to TimescaleDB')
